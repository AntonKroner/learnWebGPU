...
static void limitsSet(
  ...
  required->limits.maxBufferSize = 150000 * sizeof(Vertex);
...
bool basic3d_texturing_sampler() {
  bool result = false;
  WGPUInstanceDescriptor descriptor = { .nextInChain = 0 };
  WGPUInstance instance = 0;
  GLFWwindow* window = 0;
  const Model model = Model_load(RESOURCE_DIR "/texturing/fourareen/fourareen.obj");
  if (!glfwInit()) {
    ...
    WGPUShaderModule shaderModule =
      device_ShaderModule(device, RESOURCE_DIR "/texturing/sampler.wgsl");
